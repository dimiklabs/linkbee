        ...
        <div class="d-flex align-items-center gap-4 mb-4">
          ...
          <div class="flex-shrink-0">
            <div
              ...
              :style="{ width: '64px', height: '64px', fontSize: '1.5rem', backgroundImage: avatarPreviewUrl ? `url(${avatarPreviewUrl})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: avatarPreviewUrl ? 'transparent' : '#635bff' }"
            >
              <span v-if="!avatarPreviewUrl">{{ authStore.userInitials }}</span>
            </div>
          </div>
          <div class="flex-fill">
            <label class="form-label small fw-medium mb-1">Profile picture URL</label>
            <input
...
async function saveProfile() {
  ...
  try {
    await authApi.updateProfile({
      ...
      profile_picture: profileForm.value.profile_picture.trim() || undefined,
    });
