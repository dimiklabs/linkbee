...
async function addSocialLink() {
  ...
  let url = p.inputType === 'username' ? p.urlBase + raw : raw;
  // Ensure the URL always has a scheme so the backend `url` validator passes
  if (!/^https?:\/\//i.test(url)) {
    url = 'https://' + url;
  }
  try {
...
async function addLink() {
  ...
  try {
    const res = await bioApi.createLink({ title: newTitle.value.trim(), url });
    if (res.data) {
      links.value.push(res.data);
      newTitle.value = '';
      newUrl.value = '';
